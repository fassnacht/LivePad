...
void ListModelBase<ItemType>::appendRow(ItemType* item)
{
    ...
    connect(item, &BaseItem::dataChanged, this, &ListModelBase<ItemType>::itemDataChanged);
...
void ListModelBase<ItemType>::clear()
{
    ...
    while(!m_dataList.isEmpty()) {
        auto i = m_dataList.takeFirst();
         disconnect(i, &BaseItem::dataChanged, this, &ListModelBase<ItemType>::itemDataChanged);
        delete i;
    }
    endRemoveRows();
}
...
bool ListModelBase<ItemType>::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    for(int i=0; i<count; ++i)
    {
       ...
       disconnect(j, &BaseItem::dataChanged, this, &ListModelBase<ItemType>::itemDataChanged);
...
ItemType *ListModelBase<ItemType>::takeRow(int row)
{
    ...
    disconnect(item, &BaseItem::dataChanged, this, &ListModelBase<ItemType>::itemDataChanged);
    endRemoveRows();
    return item;
}
